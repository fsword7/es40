...
void CDEC21143::run ()
{
  try
  {
    for (;;)
    {
      ...
      asserted =
        (state.reg[CSR_STATUS / 8] & state.
         reg[CSR_INTEN / 8] & 0x0c01ffff) ? true : false;
...
CDEC21143::CDEC21143 (CConfigurator * confg, CSystem * c, int pcibus,
                      int pcidev):CPCIDevice (confg, c, pcibus, pcidev)
{
}
...
void
  CDEC21143::WriteMem_Bar (int func, int bar, u32 address, int dsize,
                           u32 data)
{
  switch (bar)
  {
...
void CDEC21143::receive_process ()
{
  ...
  if (state.reg[CSR_OPMODE / 8] & OPMODE_SR)
  {
    ...
    if (!(state.reg[CSR_OPMODE / 8] & OPMODE_OM_INTLOOP))
    {
      ...
      {
        bool resl =
          rx_queue->add_tail (packet_data, packet_header->caplen, calc_crc,
                              true);
...
//              DoClock();
  ...
  case CSR_OPMODE:             /*  csr6:  */
    ...
    data &=
      ~(OPMODE_HBD | OPMODE_SCR | OPMODE_PCS | OPMODE_PS | OPMODE_SF |
...
void CDEC21143::mii_access (uint32_t oldreg, uint32_t idata)
{
  ...
  case MII_STATE_A:
    ...
    default:
      debug
        ("[ mii_access(): UNIMPLEMENTED MII opcode %i (probably just a bug in GXemul's MII data stream handling) ]\n",
         state.mii.opcode);
      state.mii.state = MII_STATE_RESET;
    }
    state.mii.bit++;
...
//      printf("pcap recv: %d bytes (%d captured) for %02x:%02x:%02x:%02x:%02x:%02x  \n",packet_header->len, packet_header->caplen,packet_data[0],packet_data[1],packet_data[2],packet_data[3],packet_data[4],packet_data[5]);
  ...
  if (!(rdes0 & TDSTAT_OWN))
  {
    ...
    state.reg[CSR_STATUS / 8] =
      (state.
       reg[CSR_STATUS / 8] & ~STATUS_RS) | STATUS_RU | STATUS_RS_SUSPENDED;
    return 0;                   // indicate nothing was processed
  }
...
int CDEC21143::dec21143_tx ()
{
  ...
  tdes3 =
    descr[12] + (descr[13] << 8) + (descr[14] << 16) + (descr[15] << 24);
  ...
  /*  Assume no error:  */
  tdes0 &=
    ~(TDSTAT_Tx_UF | TDSTAT_Tx_EC | TDSTAT_Tx_LC | TDSTAT_Tx_NC | TDSTAT_Tx_LO
      | TDSTAT_Tx_TO | TDSTAT_ES);
...
//              fatal("{ TX: data packet: ");
    ...
    else
    {
      ...
      if (state.tx.cur_buf == NULL)
        fatal
          ("[ dec21143: WARNING! tx: middle segment, but no first segment?! ]\n");
    ...
    do_pci_read (bufaddr, state.tx.cur_buf + state.tx.cur_buf_len, 1,
                 bufsize);
  ...
  /*  Error summary:  */
  if (tdes0 &
      (TDSTAT_Tx_UF | TDSTAT_Tx_EC | TDSTAT_Tx_LC | TDSTAT_Tx_NC |
...
void CDEC21143::SetupFilter ()
{
  ...
  for (i = 0; i < 16; i++)
  {
    ...
    sprintf (mac_txt[i], "%02x:%02x:%02x:%02x:%02x:%02x", mac[i][0],
