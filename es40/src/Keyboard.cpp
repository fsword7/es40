...
CKeyboard::CKeyboard (CConfigurator * cfg, CSystem * c):CSystemComponent (cfg,
                  c)
{
  if (theKeyboard != 0)
...
void CKeyboard::enQ (u8 scancode)
{
  ...
  tail =
    (state.kbd_internal_buffer.head +
     state.kbd_internal_buffer.num_elements) % BX_KBD_ELEMENTS;
    ...
    case 0x60:                 // write command byte
      {
        ...
        bool scan_convert, disable_keyboard, disable_aux;
...
void CKeyboard::write_64 (u8 value)
{
  ...
  case 0x20:                   // get keyboard command byte
    ...
    command_byte =
      ...
      (0 << 3) |
      (state.status.sysf << 2) |
      (state.allow_irq12 << 1) | (state.allow_irq1 << 0);
  ...
  case 0xd0:                   // read output port: next byte read from port 60h
    ...
    controller_enQ ((state.irq12_requested << 5) |
...
#if defined(DEBUG_KBD)
    double cps =
      1 / ((double) (8 + (value & 0x07)) *
           (double) exp (log ((double) 2) * (double) ((value >> 3) & 0x03)) *
           0.00417);
...
//        if (bx_dbg.mouse)
    ...
    case 0xeb:                 // Read Data (send a packet when in Remote Mode)
      ...
      mouse_enQ_packet (((state.mouse.button_status & 0x0f) | 0x08), 0x00, 0x00, 0x00); // bit3 of first byte always set
...
void CKeyboard::mouse_enQ (u8 mouse_data)
{
  ...
  {
    BX_ERROR (("[mouse] internal mouse buffer full, ignoring mouse data.(%02x)", (unsigned) mouse_data));
    return;
  }
  ...
  tail =
    (state.mouse_internal_buffer.head +
     state.mouse_internal_buffer.num_elements) % BX_MOUSE_BUFF_SIZE;
