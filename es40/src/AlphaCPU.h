...
//  CTranslationBuffer * get_tb(bool bIBOX);
  ...
  int
    virt2phys (u64 virt, u64 * phys, int flags, bool * asm_bit,
               u32 instruction);
...
inline int CAlphaCPU::get_icache (u64 address, u32 * data)
{
  ...
  if (icache_enabled)
  {
    ...
    *data =
      endian_32 (state.icache[state.next_icache].
                 data[(address >> 2) & ICACHE_INDEX_MASK]);

#ifdef IDB
    current_pc_physical =
      state.icache[state.next_icache].p_address +
 ...
 **/
       ...
       {
       if (state.irq_h_timer[i]) state.check_timers = true;}
       }
       }
 ...
 **/
       ...
       state.pc += 4;
       state.pc_phys += 4;
       if (state.rem_ins_in_page) state.rem_ins_in_page--;}
 ...
 **/
       ...
       {
       state.pc = p_pc; state.rem_ins_in_page = 0;}
 ...
 **/
       ...
       {
       state.pc += a_pc; state.rem_ins_in_page = 0;}
 ...
 **/
       ...
       {
       if (translate) return state.r[RREG (i)];
       else
       return state.r[i];}
